...
2. Follow `country=Canada` through `use-report-view.ts`, the route handler, and `people-service.ts`.
3. Inspect `people-provider.ts`: identify the request, JSON validation, and five-minute cache. Find where extra login fields are removed.
4. Inspect `people-options.ts`: find how a row's count is connected to a bar. Then find how a click changes the URL.
